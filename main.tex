...
\department{Eau Environnement et Climat}
\filiere{Génie de l'Eau et de l'Environnement}

\supervisor{
    \hspace{0.5cm}$\blacktriangleright$ Encadrante interne : Mme AHATTAB Jihane (EHTP)\\[0.1cm]
    \hspace{0.5cm}$\blacktriangleright$ M. SOULI Hamza (EHTP)\\[0.1cm]
    \hspace{0.5cm}$\blacktriangleright$ Encadrante externe : Mme BENABDELLAH Khaoula (CID)
}
...
\jury{
    ...
    \hspace{0.5cm}$\blacktriangleright$ M. BENAZIZ Youssef (LPEE) \dotfill Examinateur
}
...
\AtBeginDocument{%
  ...
  \pagestyle{fancy}
  \fancyhf{}
  \fancyhead[L]{\includegraphics[height=1.0cm]{logos/cid-logo.jpg}}
  \fancyhead[R]{\includegraphics[height=1.0cm]{logos/ehtp-logo.png}}
  \fancyfoot[C]{\thepage}
...
% --- Pages préliminaires ---
\input{chapters/0_dedicace}
\input{chapters/0_remerciements}
...
% ---- Liste des acronymes ----
\input{chapters/0_acronymes}

\pagenumbering{arabic}
\setcounter{page}{1}

\input{chapters/0_intro_generale}

% --- PREMIÈRE PARTIE : ÉTUDE BIBLIOGRAPHIQUE ---
\part{PREMIÈRE PARTIE : ÉTUDE BIBLIOGRAPHIQUE}

\input{chapters/1_fondements_cfd}
\input{chapters/2_turbulence}
\input{chapters/3_approximations_numeriques}
\input{chapters/4_barrage_ahl_souss}

% --- DEUXIÈME PARTIE : REVUE DES LOGICIELS CFD ---
\part{DEUXIÈME PARTIE : REVUE DES LOGICIELS CFD}

\input{chapters/5_benchmark_logiciels}
...
\input{chapters/6_structure_cfd_openfoam}
\input{chapters/7_dimensionnement_theorique}
\input{chapters/8_mise_en_oeuvre_openfoam}
\input{chapters/9_resultats_simulation}
